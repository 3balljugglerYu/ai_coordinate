-- ===============================================
-- 公開停止時に投稿を未投稿へ戻し、再投稿を DB で塞ぐ
-- ===============================================
-- 計画: docs/planning/post-moderation-notification-implementation-plan.md
--       REQ-011, REQ-013 の改訂（実機確認を踏まえた UX 変更）
--
-- 【背景】
-- 実機確認で、公開停止された投稿が投稿者自身のギャラリーに「公開停止」バッジ付きで
-- 残り続け、詳細画面も開けない状態がユーザー体験として重いことが分かった。
-- 恒久的なラベルは離脱要因になる。画像自体は投稿者のものなので、公開停止時は
-- 「未投稿」状態に戻して普通に扱えるようにする方が自然である。
--
-- 【変更後の状態】
--   reject 時:   moderation_status = 'removed'  かつ  is_posted = false
--   → 他ユーザーからは見えない（is_posted=false のため）
--   → 投稿者は詳細画面を開ける（getPostDetail は is_posted=false を所有者に許可し、
--      moderation_status の判定は is_posted=true のときだけ評価するため既存ロジックで通る）
--   → moderation_status は 'removed' のまま維持する。異議申立ての guard trigger と
--      RPC がこれを前提にしているため
--
-- `posted_at` は NULL にしない。認容による復帰時に投稿日時とフィードの並び順を
-- 元に戻せるようにするため。
--
-- 【再投稿の禁止（重要）】
-- 投稿の公開は features/generation/lib/database.ts の postImage が
-- **ブラウザクライアントから generated_images を直接 UPDATE** して行う。
-- moderation_status を見ていないため、is_posted=false に戻すだけでは
-- 投稿者が公開停止された画像をそのまま再公開できてしまい、モデレーションが
-- 実質無効化される。クライアントのダイアログは UX であって権限境界ではない。
--
-- そこで BEFORE INSERT/UPDATE trigger で
--   moderation_status = 'removed' かつ is_posted = true
-- の組み合わせを禁止する。PostgREST を直接叩いても再公開できない。
--
-- 認容（overturn）は同一 UPDATE で moderation_status='visible' と is_posted=true を
-- 同時にセットするため、NEW.moderation_status が 'visible' となり trigger を通る。
-- service_role の例外指定は不要。

BEGIN;

CREATE OR REPLACE FUNCTION public.enforce_no_publish_while_removed()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
...
AS $$
BEGIN
  IF NEW.is_posted = true AND NEW.moderation_status = 'removed' THEN
    RAISE EXCEPTION 'post_suspended_cannot_publish'
      USING ERRCODE = '42501',
            HINT = 'This content is suspended. Publishing requires a successful appeal.';
  END IF;
...
COMMENT ON FUNCTION public.enforce_no_publish_while_removed() IS
  '公開停止 (moderation_status=removed) の投稿を is_posted=true にできないようにする backstop。異議認容は moderation_status も同時に visible へ変えるため通る。';

DROP TRIGGER IF EXISTS trg_enforce_no_publish_while_removed ON public.generated_images;
CREATE TRIGGER trg_enforce_no_publish_while_removed
  BEFORE INSERT OR UPDATE OF is_posted, moderation_status ON public.generated_images
  FOR EACH ROW
  EXECUTE FUNCTION public.enforce_no_publish_while_removed();

REVOKE ALL ON FUNCTION public.enforce_no_publish_while_removed() FROM PUBLIC, anon;

-- ── reject 時に is_posted=false へ戻す ──────────────────────────
...
BEGIN
  ...
  -- ロック獲得後の再検索（並行再送でも既存 decision id を返す）
  SELECT id INTO v_existing_decision_id
  FROM public.moderation_audit_logs
  WHERE action IN ('approve', 'reject')
    AND metadata->>'idempotency_key' = p_idempotency_key::text
  ...
  -- reject では is_posted を false に戻し、投稿者のギャラリーで未投稿として
  -- 普通に扱えるようにする。posted_at は保持して復帰時に並び順を戻せるようにする。
  ...
  SET
    ...
    is_posted = CASE WHEN p_action = 'approve' THEN is_posted ELSE false END
  WHERE id = p_post_id;
...
BEGIN
  ...
  IF p_action = 'overturn' THEN
    -- 公開停止時に is_posted=false へ戻しているため、復帰では両方を戻す。
    -- moderation_status と is_posted を同一 UPDATE で設定するので、
    -- enforce_no_publish_while_removed は NEW.moderation_status='visible' を見て通す。
    -- posted_at は公開停止時に保持しているので、元の並び順に戻る。
    UPDATE public.generated_images
...
-- ===============================================
-- DOWN:
-- trigger の撤去は再投稿を許してしまうため行わない。
-- 表示方針だけ戻したい場合はアプリ側 (MyImageCard) の変更を revert する。
-- ===============================================
