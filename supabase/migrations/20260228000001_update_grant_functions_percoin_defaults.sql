-- grant 系 RPC を percoin_bonus_defaults / percoin_streak_defaults 参照に変更
...
BEGIN
  v_amount := get_percoin_bonus_default('tour_bonus');
  ...
  BEGIN
    INSERT INTO public.notifications (
      ...
      'チュートリアルボーナス獲得！',
      'チュートリアル完了で' || v_amount || 'ペルコインを獲得しました！',
...
DECLARE
  ...
  v_expire_at TIMESTAMPTZ;
BEGIN
  v_bonus_amount := get_percoin_bonus_default('daily_post');
...
BEGIN
  ...
  v_bonus_amount := get_percoin_streak_amount(v_new_streak_days);
