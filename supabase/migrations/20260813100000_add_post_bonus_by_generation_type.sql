...
-- ## 適用順序: **デプロイ → migration**
--
-- 本 migration は EXECUTE を service_role のみへ絞る。現行 main の投稿APIは
-- session client から呼んでいるため、**migration を先に当てると REVOKE 後の
-- 権限エラーを握りつぶして bonus_granted=0 になる**(投稿は成功するので気づかない)。
--
-- 逆順(デプロイが先)は安全。新しいアプリは admin client = service_role で呼び、
-- service_role の EXECUTE は旧定義にも付いているため、migration 適用までは
-- 旧仕様(生成方法を問わず1日1回)で正しく付与され続ける。
--
-- デプロイから migration までの間だけ、ミッション画面の投稿ボーナス行が
-- 出ない(daily_post_bonus_grants と get_post_bonus_amounts がまだ無く、
-- 額が空になるため行が0件になる)。付与は動いており、見た目だけの一時的なもの。
-- 間隔は短くすること。
--
