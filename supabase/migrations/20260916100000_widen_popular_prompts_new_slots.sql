-- 🔥人気のプロンプト: 新着枠を 3 件 → 7 件に広げ、差し込み位置を 2〜14 番目にする。
--
-- 背景: 「直近 24 時間から 3 件」という仕様だったが、対象投稿が 1 日 6〜13 件
-- あるため 24 時間の窓は一度も効いておらず、実測で 🆕 が付く時間は平均 9.5 時間
-- (丸 24 時間付いた投稿は過去 30 日で 0 件)。上限を決めていたのは LIMIT 3 の方。
--
-- 7 件にすると平均 20.0 時間まで伸び、214 件中 83 件が丸 24 時間 🆕 を持てる。
-- 差し込み幅も 2〜9 から 2〜14 へ広げ、上位 20 位の中に 7 件が収まるようにする
-- (幅を変えないと上位 9 位が新着で埋まる)。
--
-- スコアの定義そのものは変更しない。変えたのは新着枠の 2 つの数だけ。
...
BEGIN
  ...
  -- 採った順に 1 つずつずらして差し込む。位置は 2 + floor(r * 13) = 2〜14 番目。
  --
  -- ⭐ **挿入位置と最終位置は違う。** 後から差し込んだものが、先に入った
  --    ものを 1 つずつ下へ押し出す。3 件の頃も挿入幅は 2〜9 だったが、
  --    実際の順位表では 11 位に出ていた。
  --
  --    「上位 20 位の中に 7 件」を満たす幅を実測で選んだ(20万回試行)。
  --
  --      挿入幅 2〜14 → 上位20に 7.00 件 (最悪でもちょうど 20 位)  ← これ
  --      挿入幅 2〜16 → 上位20に 6.80 件 (最悪 22 位)
  --      挿入幅 2〜20 → 上位20に 5.99 件 (最悪 26 位)
  --
  --    件数を増やすときは、この幅も一緒に見直すこと。片方だけ動かすと
  --    上位が新着で埋まるか、逆に 🆕 が 2 ページ目へ落ちる。
...
-- ===============================================
-- DOWN: 20260902110000 の定義へ戻す (LIMIT 3 / * 8)
-- ===============================================
