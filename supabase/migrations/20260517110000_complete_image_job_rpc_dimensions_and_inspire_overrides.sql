-- complete_image_job_with_generated_images の INSERT 内容を最新スキーマへ再同期する。
--
-- 背景:
-- - 20260510150000_store_generated_image_dimensions.sql は OpenAI batch 結果の
--   width / height を generated_images に保存するよう RPC を更新した。
-- - origin/main の 20260516120000_inspire_override_checkboxes.sql は後続で同じ RPC を
--   再定義し、override_outfit / override_angle / override_pose / override_background を
--   COPY するようにした。
--
-- この migration は両方を統合し、後勝ちの RPC 定義で片方が落ちないようにする。
