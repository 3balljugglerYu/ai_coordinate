...
--
-- 【デプロイ順序 (REQ-021) — 重要】
-- 本マイグレーションは「稼働中のコードから権限を剥がす」方向のため、
-- 必ず **ルート側の admin クライアント差し替え (PR #456) がデプロイ完了した後**
-- にマージすること。
--
-- このリポジトリでは Supabase の GitHub 連携により、main へのマージ時点で
-- マイグレーションが本番へ自動適用される。つまり「マージ = 適用」である。
-- したがって PR #456 のデプロイ完了前に本PRをマージすると、旧コード
-- (セッションクライアントから RPC を呼ぶ) が権限エラーになり、運営通報と
-- しきい値到達通報が 500 になる。
--
-- 正しい順序:
--   1. PR #456 をマージ
--   2. Vercel のデプロイ完了を確認
--   3. 本PRをマージ (= この時点で REVOKE が適用される)
