...
-- 新テーブルは generated_images / image_jobs / auth.users への FK を持つため、
-- 作成時に参照先へ SHARE ROW EXCLUSIVE ロックがかかる。新テーブルは空で
-- 既存データの検証が走らないためロックは一瞬だが、長いトランザクションが
-- 先行している場合に生成・投稿の書き込みが詰まることを避けるため上限を置く。
-- 取れなければこのトランザクションごと失敗し、何も適用されずに終わる。
