-- Allow 'wardrobe' as a signup_source (guest "保存する" → 新規登録 attribution).
-- Extends the existing 'style' attribution added in 20260412204000.
...
CHECK (
  ...
  OR signup_source = ANY (ARRAY['style'::text, 'wardrobe'::text])
);

COMMENT ON COLUMN public.profiles.signup_source IS
  'Initial signup source attribution. Tracks style-origin and wardrobe-save (login-conversion) registrations.';

-- handle_new_user の流入元バリデーションを 'style' / 'wardrobe' に拡張する。
-- (本文は 20260412204000 の定義を踏襲し、検証行のみ更新)
...
BEGIN
  ...
  IF v_signup_source IS NOT NULL
     AND v_signup_source NOT IN ('style', 'wardrobe') THEN
