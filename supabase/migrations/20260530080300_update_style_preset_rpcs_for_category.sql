-- style_presets mutation RPC を category / image_input_mode / reference_image_* 対応へ拡張する。
-- 設計判断は docs/planning/style-preset-raw-mode-implementation-plan.md Phase 2 参照。
...
COMMENT ON FUNCTION public.create_style_preset(
  ...
  'style_presets の作成と表示順調整を原子的に行い、styling/background prompt + category + image_input_mode + reference image を保存する';
COMMENT ON FUNCTION public.update_style_preset(
  UUID, TEXT, TEXT, TEXT, TEXT, TEXT, INTEGER, INTEGER, INTEGER, TEXT, UUID,
  UUID, TEXT, TEXT, TEXT, INTEGER, INTEGER
) IS
  'style_presets の更新と表示順調整を原子的に行い、styling/background prompt + category + image_input_mode + reference image を保存する';
