...
// Edge Function の実行時間内に収めるため、Retry-After が長くても 30 秒で打ち切る。
...
/**
 * OpenAI gpt-image-2 を呼び出して画像編集を実行（`quality` と `sizeTier` は呼び出し側指定）。
 *
 ...
 * - data[].b64_json が空 → "No images generated"（既存 isNonRetriableGenerationError で即終端 failed）
 *
