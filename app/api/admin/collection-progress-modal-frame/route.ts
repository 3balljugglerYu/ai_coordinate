...
/**
 * POST /api/admin/collection-progress-modal-frame
 * 進捗モーダル(CollectionProgressModal)の土台フレーム画像をアップロードする。
...
export async function POST(request: NextRequest) {
  ...
  const path = `collection-progress-modals/${categoryKey}/${randomUUID()}.${ext}`;
  const { error: uploadError } = await admin.storage
    .from(BUCKET)
    .upload(path, buffer, { contentType, upsert: false });
  if (uploadError) {
    console.error("[collection-progress-modal-frame] upload failed:", uploadError);
    return NextResponse.json(
      { error: "アップロードに失敗しました" },
      { status: 500 },
    );
