...
/**
 * POST /api/admin/collection-character
 * 進捗リング中央に表示する「シリーズ用キャラ画像(名前なし)」をアップロードする。
 * ユーザー表示用のため public バケット(generated-images)に保存し、保存パスを返す。
...
export async function POST(request: NextRequest) {
  ...
  if (!ext) {
    return NextResponse.json(
      { error: "PNG / JPEG / WebP 画像のみ対応しています" },
      { status: 400 },
    );
  }
  ...
  const path = `collection-characters/${categoryKey}/${randomUUID()}.${ext}`;
  const { error: uploadError } = await admin.storage
    .from(BUCKET)
    .upload(path, buffer, { contentType, upsert: false });
  if (uploadError) {
    console.error("[collection-character] upload failed:", uploadError);
