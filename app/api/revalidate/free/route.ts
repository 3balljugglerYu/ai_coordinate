...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    revalidateTag(`free-${user.id}`, "max");
    // revalidatePath: ページ全体のキャッシュを即時無効化（router.refresh で最新データを取得するため）
    revalidatePath("/free");
    return NextResponse.json({ success: true });
  } catch (error) {
    console.error("Revalidate free error:", error);
    return jsonError(copy.freeFailed, "REVALIDATE_FREE_FAILED", 500);
  }
}
