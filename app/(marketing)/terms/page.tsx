          ...
          sections: [
            ...
            },
            {
              ...
              list: [
                ...
                "第三者の著作物、トレードドレス、商標、キャラクター等を、当該権利者の利用規定・ガイドラインに違反する形でプロンプト投入・スタイリングカード作成・共有・公開する行為（商用利用または営利目的での利用を含みます）",
                ...
                "本サービスの趣旨や雰囲気を著しく損なうコンテンツの投稿。具体的には、過度に恐怖・不快感を与える描写、政治的・宗教的に偏った主張や論争を煽る表現、コーデや AI スタイリング要素を含まない投稿および特定ブランド・商品の宣伝に偏った投稿等、本サービスのコーディネート文脈と乖離した表現等を含みますが、これらに限られません。",
            ...
            },
            {
              ...
              paragraphs: [
                ...
                "当サービスは、生成モデル側の安全フィルタが状況に応じて個別に設定される場合があり、その場合でもコミュニティガイドラインで禁止しているコンテンツ（児童描写、実在人物の非同意ヌード、極端な暴力等）を生成・投稿することは一切認めません。生成モデルが結果を返した場合でも、その出力が本規約・コミュニティガイドラインに違反する場合は禁止事項に該当します。",
            ...
            },
            {
              ...
              paragraphs: [
                "当サービスでは、ペルコインを購入により取得するほか、新規登録・チュートリアル完了・紹介特典・デイリー投稿・連続ログイン・サブスクリプションプランの月次特典などの形で無料で付与することがあります。",
                "購入により取得したペルコインには有効期限は設定しておりません。",
                "無料で付与したペルコインには有効期限があり、付与した月の翌月から起算して6ヶ月後の月末（日本時間）をもって失効します。失効した無料ペルコインの返還・再発行は行いません。",
                "サブスクリプションプランの月次特典として付与されるペルコインは「無料付与分」として扱われ、付与日から所定の期間（現在は6ヶ月）が経過した時点で失効します。一方、当サービスのチャージ機能を通じて購入されたペルコインは、原則として有効期限なくご利用いただけます（詳細は「資金決済法に基づく表示」内の「ペルコインの有効期限について」もあわせてご確認ください）。",
            ...
            },
            {
              ...
              description: "通報・ブロック機能、違反対応、異議申立てについて",
              paragraphs: [
                "当サービスは、不適切なコンテンツの排除およびユーザー間のトラブル防止のため、通報・ブロック・コンテンツ審査の機能を提供しています。執行の運用は、コミュニティガイドラインおよび本条に従うものとします。",
              ],
              list: [
                "通報：ユーザーは、他のユーザーの投稿が利用規約・コミュニティガイドラインまたは法令に反する等と判断した場合、所定の方法で通報することができます。通報された投稿は、通報の重み付けが一定のしきい値に達した時点で審査待ちとなり、審査が完了するまで当サービス上で全ユーザーに非表示となる場合があります。通報者の情報は、通報された投稿の投稿者に開示しません。",
                "ブロック：ユーザーは、他のユーザーをブロックすることができます。ブロックしたユーザーの投稿は、ホーム・タイムライン・検索結果においてユーザーの画面に表示されません（投稿詳細ページやコメント・返信欄など、表示制限が及ばない場所もあります）。ブロック関係は、アカウント画面から解除できます。",
                "違反対応：当サービスは違反の重大性、反復性、悪質性等を勘案し、当該コンテンツの非公開・削除、またはアカウントの停止等の措置を講じます。CSAM、深刻な権利侵害、明確な犯罪行為、重大な安全リスクが認められる場合は、即時に厳格な措置を講じます。なお、運用の改善に伴い、警告制度や段階的な機能制限など、より細やかな対応手段を順次導入していく予定です。",
                "アカウント停止：違反の程度に応じて、運用上、一時的な停止と永続的な停止を使い分けます。停止された場合、ログイン後の機能の利用が制限されることがあります。永続的な停止に該当した場合、再登録はお断りすることがあります。停止の判断および期間は当サービスが決定します。なお、停止期間の自動管理や再登録防止の仕組みは、順次強化していく予定です。",
                "異議申立て：投稿の公開停止措置を受けたユーザーは、措置の通知から原則として14日以内に異議を申し立てることができます。申立ては、公開停止の通知から遷移する判定内容の画面で行えます。同画面では、実施された措置とその期間、違反したポリシー、運営からの説明、審査の経過と結果を確認できます。公開停止された投稿は投稿前（未投稿）の状態に戻り、生成画像自体は削除されませんが、異議申立てが認められない限り再度投稿することはできません。当サービスは合理的な期間内に再審査を行い、認容・棄却のいずれの場合も理由を添えて結果を通知します。認容した場合は投稿の公開を再開します。通報者が誰であるかは、いかなる場合も投稿者に開示しません。CSAM 等明白かつ重大な違反については、異議申立てを受け付けた場合でも、再審査の結果これを認めないことがあります。",
