...
export function buildCollectionSummaryCsv(
  ...
  const rows: (string | number)[][] = [
    ...
    uuRow("訪問UU(ログイン)", uuFunnel.visitsMemberUu),
    uuRow("訪問UU(ゲスト)", uuFunnel.visitsGuestUu),
    uuRow("生成UU(ゲスト)", uuFunnel.generatesGuestUu),
    uuRow("ゲスト訪問→生成率(%)", pct(uuFunnel.guestGenerateRatePct)),
