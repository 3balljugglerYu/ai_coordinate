...
import { Loader2, Maximize2, Minimize2, Share2 } from "lucide-react";
...
/**
 * フォームの状態のうち、外側(/style のスタイル一覧)が知る必要のあるもの。
 * 一覧は生成中・ゲストの結果固定中にカードを押せなくし、
 * お気に入りやゲスト向けのカード表示にログイン状態を使う。
 */
export interface OneTapStyleGenerationFormStatus {
...
/** 外側から呼べる操作(ref 経由)。 */
export interface OneTapStyleGenerationFormHandle {
  /**
   * 結果があれば「結果を消してよいか」を確認し、同意されたら action を実行して
   * エラーと結果を消す。/style でスタイルを切り替えるときに使う。
   */
  requestChange: (action: () => void) => void;
  /** 認証モーダルを開く(未開放スタイルの案内の「ログイン」から)。 */
  openAuthModal: () => void;
}

interface OneTapStyleGenerationFormProps {
  /** 生成するスタイル。/style では選択中のもの、生成シートでは押したものに固定。 */
  preset: StylePresetPublicSummary | null;
  /**
   * どこに置かれるか。ページ前提の処理(チュートリアルの目印・ジョブの再開・
   * ページ全体の再読み込み・スクロール・完了トースト)は "page" のときだけ行う。
   * 計画書: docs/planning/styles-generation-sheet-implementation-plan.md §10-2
   */
  variant: "page" | "sheet";
  ...
  /**
   * 利用イベント(generate)の categoryKey を styleId から引く。
   *
   * /style はジョブの再開で、選択中とは別のスタイルの結果を確定しうる。
   * そのときは選択中のスタイルではなく、確定したスタイルのカテゴリで記録する
   * 必要があるため、ページは一覧から引く関数を渡す。
   * 未指定なら、styleId が preset と一致するときだけ preset のカテゴリを返す。
   */
  resolveCategoryKey?: (styleId: string) => string | null;
  /** 状態が変わったときに外側へ知らせる。 */
  onStatusChange?: (status: OneTapStyleGenerationFormStatus) => void;
  ref?: Ref<OneTapStyleGenerationFormHandle>;
}
...
/**
 * One-Tap Style の生成フォーム(スタイル1つに対する入力・生成・結果)。
 *
 * `/style` ページ(`StylePageClient`)と、`/styles` の生成シート(`StyleGenerationSheet`)で
 * 共用する。フォームを2つ持つと片方だけ直して壊す事故が起きるため、同じ部品を使う。
 * スタイルの選択(ストリップ・探索シート)は外側の責務で、ここは渡された `preset` だけを扱う。
 *
 * もとは `StylePageClient` の一部で、コードは書き換えずに移した(計画書 ADR-009)。
 */
export function OneTapStyleGenerationForm({
  preset,
  variant,
  initialAuthState,
  requestedModel = null,
  ...
  onStatusChange,
  ref,
}: OneTapStyleGenerationFormProps) {
  // ページ前提の処理を行うか(§10-2)。シートの中ではページを動かさない。
  ...
  // もとの StylePageClient の変数名を保つ(移したコードを書き換えないため)。
  ...
  }, [isGenerating, isGuestResultLocked, effectiveAuthState, onStatusChange]);
  ...
  useEffect(() => {
    // シートの中では scrollIntoView が背後のページまで動かしうるため行わない(ADR-004)
    if (!isPageVariant || generationPhase !== "running") {
      return;
    }

    scrollSectionIntoView(generationStatusSectionRef.current);
  }, [generationPhase, isPageVariant]);

  useEffect(() => {
    // シートでは結果が同じシートの一覧にすぐ出るので、完了トースト(押すと結果へ
    // スクロール)は出さない
    if (!isPageVariant || generationPhase !== "completing") {
      return;
    }
  ...
  }, [dismiss, generationPhase, isPageVariant, t, toast]);
  ...
  /*
    外側(/style のスタイル一覧)から呼ぶ操作。スタイルの切り替えは、もとは
    handlePresetSelect(ここにあった)で「結果リセットの確認 → 選択 → エラーと結果を消す」
    をしていた。選択の状態は一覧側に残るので、確認とリセットだけをここで引き受ける。
  */
  ...
  /*
    /style のジョブ再開キー(sessionStorage)を消す。シートはこのキーを
    読みも書きもしない(ADR-003)。シートから消すと、/style で走っている
    別のジョブの再開を消してしまう。
  */
  const clearPersistedStyleJob = () => {
    if (isPageVariant) {
      clearActiveStyleJob();
    }
  };
  ...
  const finalizeStyleAsyncSuccess = (
    ...
    // シートでは /style の一覧の無効化だけ行う。router.refresh は置かれている
    // ページ(/styles)全体を取り直すだけで、シートの一覧には効かない。
    void (async () => {
      try {
    ...
    if (styleId) {
      /*
        ...
        (presets を持つのはページなので、引き方は resolveCategoryKey で受け取る)
      */
      const generatedCategoryKey = resolveCategoryKey
        ? resolveCategoryKey(styleId)
  ...
  useEffect(() => {
    // シートは /style の再開キーを読まない。拾うと /style のジョブを
    // シートで確定したり、完了を二重に知らせたりする(ADR-003)
    if (!isPageVariant) {
      return;
    }
  ...
  const generateImageWithAsyncJob = async () => {
    ...
    try {
      ...
      // シートでは保持しない。閉じたら全体の生成中バーに引き継ぐ(ADR-003)。
      if (isPageVariant) {
        persistActiveStyleJob({
          jobId: payload.jobId,
          styleId: selectedPreset.id,
  ...
  /*
    ルートは Fragment にする。/style ではページの space-y-8 の中に、もとと同じ
    並び(スタイル選択の section の次にこの section、エラー、結果…)で入る。
  */
  return (
    <>
      <section className="space-y-6">
        {/* チュートリアル Step2: 見出しごとハイライトする(シートには目印を置かない) */}
        <div
          data-tour={isPageVariant ? "style-tour-character" : undefined}
          className="space-y-6"
        >
            ...
            >
              ...
              {selectedPreset ? (
                <StyleReferencePanel
                  ...
                  // シートは開いた瞬間に画像を読み始めるので、読み込むまでスケルトンを出す
                  showLoadingSkeleton={!isPageVariant}
        ...
        <Card className="p-6">
          <div className="space-y-6">
            ...
            */}
            ...
            <div data-tour={isPageVariant ? "style-tour-generate" : undefined}>
              {isGuestRestrictedCategory ? (
                <div className="space-y-2">
                  <Button
                    type="button"
