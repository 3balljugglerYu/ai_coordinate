...
お知らせで「生成から24時間以内が対象」と予告してあるので、
**驚きではなく納得**にする。ここを告知せずに出すと問い合わせになる。

### ⑤ 夜中に作って日付をまたいで投稿する人（ここが分かれ目）

23:55 にフリースタイルで生成し、キャプションを考えていたら 00:10 になった。
そのまま投稿する。

**+20 が付く**（生成から15分しか経っていないため）。
もし条件を「JST当日に生成」にしていたら、ここで付かなくなる。
**「さっき作ったのに付かない」を避けるための判断**（ADR-004b）。
...
- **REQ-05b**: **生成から24時間以内**の画像のみ付与対象とする。それより古い画像を
  投稿しても付与しない（在庫の消化で毎日受け取れる状態をなくす）。
...
### ADR-004b: 鮮度の条件は「JST当日」ではなく「生成から24時間以内」

- **Context**: 現在は何日前に生成した画像でも投稿すれば付与される。在庫を1枚ずつ
  出せば毎日受け取れてしまう。ユーザーから「24時間以内」か「JST当日に生成」の
  どちらかにしたい、判断は実装の容易さで、との指定。
- **Decision**: **生成から24時間以内**（`created_at > now() - interval '24 hours'`）。
- **Reason**: **実装コストは同じ**（RPC は生成方法を引くために投稿の行を既に読むので、
  どちらも条件1行）。そこで挙動で選んだ。「JST当日」は
  **23:55 に生成して 00:01 に投稿すると付かない**という段差ができる。
  投稿は夜に集中しやすく、「さっき作ったのに付かない」は説明のつかない体験になる。
  24時間以内なら段差がなく、ユーザーの狙い（数日前の在庫では受け取れない）は満たせる。
- **Consequence**: 一度の生成セッションから、翌日ぶんも受け取れる場合がある
  （12:00 に生成 → 当日 12:00 投稿 → 翌日 11:00 に別の1枚を投稿）。
  1日1回・生成方法ごとの上限は効くので、増えても1日ぶん。
  厳しくしたい場合は条件を「JST当日」に変えるだけ（1行）。
...
- [ ] `grant_daily_post_bonus` を書き換え（**引数は変えない**）
  ...
  - **生成から24時間以内**か確認する（`created_at`。同じ行から読めるので追加コストなし）
