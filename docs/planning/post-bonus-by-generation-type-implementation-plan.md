...
**出す順序**: お知らせ（変更予告）→ リリース → バナー掲出。
減る変更を含むので、リリースの数日前に予告する。

**リリース時の順序は「デプロイ → migration → 額の確認」**。
migration が EXECUTE を service_role へ絞るため、**先に当てると
現行アプリ（session client 呼び出し）の付与が静かに止まる**。
逆順なら、新アプリは admin client で呼ぶので migration 前も旧仕様で付与され続ける。
