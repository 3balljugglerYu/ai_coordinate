...
### ADR-010: アプリの画面は Web の**モバイル版**を正本にする

- **Context**: Phase 4 までは API と機能の同等化を追い、画面はアプリ側で組み立てていた。実機で並べると、操作の置き場所（投稿者本人の編集・取り消し）、文字の大きさ、カードの寸法、タブの見た目が Web と揃っていなかった
- **Decision**: 画面を作るときは Web の**モバイル幅**の実装を読み、寸法・配置・文言・色をそこから写す。デスクトップ表示は参照しない
- **Reason**: 同じサービスの別クライアントであり、ユーザーは行き来する。置き場所が違う機能は「無い」のと同じ。寸法や文字サイズの差は個々には小さくても、並ぶと別プロダクトに見える
- **Consequence**: 実装前に Web 側の該当コンポーネントを読む手間が増える。代わりに、Web の値（Tailwind のクラスや定数）を写す先を 1 か所に集めれば、以降の画面は自動的に揃う。ライブラリ由来の制約（例: Swiper がスライド 2 枚以下でループを無効化する）は設計判断ではないので、そこまで真似はしない
...
### 実機フィードバックによる画面の手直し（Phase 4 と Phase 5 のあいだ、2026-09-08）

目的: Phase 4 までに載せた画面を、ADR-010 のとおり Web のモバイル版に合わせる。
ビルド確認: 実機のホームと Web のモバイル版を並べて、寸法・文字・配置の差が分からないこと。

| PR | 内容 |
|----|------|
| persta-app #29 | ヘッダーとボトムナビをスクロールで隠す（X の挙動）。「Persta \| ペルスタ」の見出し・リード文・「最新アップデート」を削除。1 列フィードを既定に、before/after を Web と同じ向き別レイアウトに、One-Tap Style の棚とポップアップバナーを移植、ブランドカラーを `from-pink-500 to-orange-400` に是正 |
| persta-app #30 | 文字スケールを Web の Tailwind（12/14/16/18/20/24）で一本化。スタイルカードを Web と同じ 180×(240+44) の共通コンポーネントへ。フィードのタブをピル型チップから下線タブへ。バナーの角丸を 28 → 12、影と重ね文字を削除 |
| persta-app #31 | バナーのループ。スタイル棚がスクロールで戻ると消える問題 |

判断と残課題:

- ⭐Web の「ブランドカラー」は `--primary` ではない。`--primary` は shadcn 既定のニアブラックで、実際のブランドはピンク → オレンジのグラデーション（`from-pink-500 to-orange-400`）。ピンクは**操作**のための色なので、タブなどのナビゲーションには使わない（Web もそうなっている）
- ⭐Material の既定タイプスケールは Web の Tailwind より見出しで 1〜2 段大きい。書体は元から一致している（Tailwind の `font-sans` は `system-ui` 始まりで Flutter が各プラットフォームで使う面と同じ）ので、揃えるべきは大きさだけ
- ⭐同じカードを 2 か所で書くと必ずずれる。ホームの棚と一覧は `StylePresetPreviewCard` に集約した（Web も `fluid` で 1 つを使い回している）
- ⭐スクロールで画面外に出るセクションに autoDispose の ViewModel を持たせない。棚は unmount → 再取得で約 1 秒空白になっていた。購読は unmount しない親（ホーム）が持ち、子は presets を受け取って描くだけにする（`AutomaticKeepAliveClientMixin` は効かなかった）
- ⭐`PageView` は `padEnds: false` と `viewportFraction 1/1.5` の組み合わせで最終ページが「枚数 − 1.5」で頭打ちになる。`animateToPage(枚数-1)` が届かず、ドットは最後・画像は先頭のまま停止した。終端を無くす（無限ページ + `nextPage()`）のが正しい直し方
- バナーが 2 枚のときだけ Web と挙動が違う。Swiper は `slides >= slidesPerView + loopedSlides`（= 2.5）を満たさないとループを無効化するため、Web は 1 枚と 2 枚でループしない。ADR-010 のとおりライブラリの制約は真似せず、アプリは 2 枚でもループさせている
- 🔥人気タブは Web が運営アカウント判定、アプリはビルド時フラグ `POPULAR_PROMPTS_ENABLED`（ADR-009）。実機ビルドでは off なので出ない。どちらに寄せるかは未決
- 企画の棚（コレクション）は Phase 3 でスコープ外と決めたまま。ホームの棚としては未実装
