...
### Phase 6: 投稿時サジェスト（Phase 1〜5 マージ後・別 PR）
目的: 表記ゆれの自然収束と、タグがゼロの状態からの立ち上げ
ビルド確認: lint / typecheck / test / build + 実機（IME）

**前提となる実測（2026-08-28 本番）**: 公開投稿 1,480 件のうち `#` を含むのは **1 件**。
つまり「既存タグの前方一致」だけを作っても**候補が空**で、公開の条件を満たさない。
候補の供給源を先に用意する必要がある。

**プリセットの特定可否（実測）**: `generation_prompt_snapshots.source_revision` に
One-Tap のプリセット ID が入っている（2026-07-29 以降の生成は 100%、投稿済み 377 件が解決可）。
`image_jobs.style_template_id` と `style_usage_events` からは復元できない（どちらも 0 件）。
これ以前の投稿には記録が無いため、**過去分の掘り起こしには使えない**。

#### 6a. 企画ごとのタグ名（設定の器）

- [ ] `preset_categories.hashtag_suggestions text[]`（**最大 5 個**）を追加。
      各要素は `lib/hashtag.ts` の規則に沿った表記（`#` は含めない・空白不可・50 文字以内）
- [ ] admin のカテゴリ編集画面に入力欄。空なら候補を出さない
- [ ] 新企画は**この欄を埋めるだけ**でタグが立ち上がる（コード変更・デプロイ不要）

#### 6b. 候補の提示（押して入る。自動で付けない）

- [ ] **A: 作品の文脈から** — 投稿対象の作品 → snapshot → プリセット → カテゴリ →
      `hashtag_suggestions`。サーバーで解決して返す（プロンプト本文は返さない）
- [ ] **B: 自分が前に使ったタグ** — 直近で自分が使ったタグ。A が空のときの受け皿
- [ ] `PostModal` の説明欄の下に候補を並べ、押すと**説明文の末尾に挿入**する。
      既に入っているタグは候補から外す
- [ ] 段階公開の判定（`searchAvailable`）を通す。閉じている間は候補も出さない

#### 6c. 入力中の前方一致（既存タグ）

- [ ] `GET /api/hashtags/suggest?prefix=` — `name_normalized` 前方一致、
      **公開中の投稿での使用回数順**、上位 8 件。使用回数 0 のタグは出さない
      （取消・公開停止した投稿にしか使われていないタグが候補に残るため）
- [ ] 表示は最も使われている原文表記（`#AI` と入力しても既存タグ `#ai` を候補に出す）
- [ ] ⚠️ **日本語入力**: 変換中は候補を出さない。Enter は変換確定なので候補の挿入に使わない
      （確定したつもりでタグが入る事故になる）。`rich-textarea` の composition 状態を使う
- [ ] ⚠️ モーダル内でドロップダウンが切れないこと（実機確認）
