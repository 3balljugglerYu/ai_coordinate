...
## 運営に確認したい事項（本 PR では変更しない）

レビュー過程で判明した**既存の挙動**です。本機能とは独立しており、本 PR では現状を維持します。

- **ツアーボーナス（20）・紹介ボーナス（50）は5万キャップを通らない**。
  無料残高が49,998でも満額が付与され、5万を超えます（キャップ導入当初からの挙動）。
- これをキャップ対象に含めると「上限に達したユーザーにはボーナスを配らない」という**製品仕様の変更**になります。
  変更するか / 現状維持かは運営判断が必要なため、別タスクとして切り出せるよう記録しておきます。
- なお `grant_admin_bonus`（運営裁量）と `refund_percoins`（返金は消費分の復元）は、
  キャップを通さないことに合理性がある**意図的な例外**と考えます。
