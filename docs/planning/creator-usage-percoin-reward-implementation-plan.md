...
- **REQ-12** (状態駆動): While イベントが `legacy`（本機能導入前の既存イベント）, the system shall 再処理の対象にしない。
  過去の利用に遡って付与されることはない。
- **REQ-13** (状態駆動): While 同一の受け手へ**本機能の還元付与**が複数並行して走る, the system shall 受け手単位で直列化し、
  5万無料残高キャップを超えない（ADR-008 再改訂）。
  **保証範囲は本機能の2経路（Free 還元 / Style 還元）の間のみ**。この機能が現実に作る競合
  （人気クリエイターへの高頻度な同時付与）はここに収まる。
  既存ボーナス（完走報酬・デイリー・ストリーク・サブスク）と並行した場合の超過、および
  登録・ツアー・紹介・admin 付与・返金がそもそもキャップ計算を行わない点は、
  **いずれも本機能導入前からの既存仕様**として残る（下記「キャップ非適用の既存経路」「別タスク」を参照）。
...
### ADR-008（再改訂・実装時に確定）: 受け手単位の直列化は**還元の付与RPC 2本の冒頭**で行う

> **実装時の変更**: 当初は共有の `get_grantable_free_percoin_amount` にロックを置き、
> この関数を通る全経路を一括で直列化する設計だった。しかし本番の既存関数を確認したところ、
> **`grant_daily_post_bonus` はキャップ計算 → `profiles` UPDATE、`grant_streak_bonus` は
> `profiles` UPDATE → キャップ計算**とロック取得順が逆で、共有関数にロックを入れると
> 同一ユーザーの投稿報酬とログイン報酬が循環待ち（デッドロック）になることが判明した。
> キャップ超過は「稀に上限を少し超える」だけだが、デッドロックは**既存の報酬付与そのものを
> 失敗させる**。割に合わないため共有関数への変更は撤回し、本機能の付与RPC 2本の冒頭
> （test-and-set より前）でのみ受け手ロックを取る。
> **したがってキャップの強制は本機能の2経路間でのみ成立し、既存ボーナスとの並行時の超過は
> 本PR以前と同じ状態で残る。** 全経路統一は下記「別タスク」を参照。

**以下は当初案の記録（実装は上記のとおり変更済み。歴史的経緯として残す）。**
...
- [ ] マイグレーション `2026080615xxxx_add_creator_usage_percoin_reward.sql`
  ...
  - **`get_grantable_free_percoin_amount` は元の定義のまま**（ロックを入れない。ADR-008 再改訂）。
    初期版が入れてしまった環境のために、元の定義を明示的に `CREATE OR REPLACE` で書き戻し、
    カタログ検証でロックの不在を assert する
  - `grant_prompt_usage_reward(p_event_id uuid)` / `grant_style_preset_usage_reward(p_event_id uuid)`
    （service_role 限定 / **冒頭で受け手単位の advisory lock**（test-and-set より前）/
    `reward_status='pending'` の test-and-set /
...
- [ ] **並行制御**: **本機能の2経路間**で、同一受け手へ2セッションから同時付与してもキャップを超えないこと。
      既存ボーナスとの競合時の超過は本PRの保証外（ADR-008 再改訂。全経路統一は別タスクで検証・対応する）
...
- [ ] **既存付与の非回帰**: **キャップ関数がロックなしの元定義へ戻っている**こと（カタログ検証で assert）。
      既存4関数（完走報酬・デイリー・ストリーク・サブスク）と、キャップを通らない経路
      （登録・ツアー・紹介・admin・返金）の挙動を一切変えていないこと
...
- **全付与経路への受け手ロック統一**（ADR-008 再改訂）: キャップを全経路の厳密な強制上限にするには、
  `get_grantable_free_percoin_amount` を通る既存4関数（完走報酬・デイリー・ストリーク・サブスク）で
  **「受け手ロックを最初に取る」順序へ統一**する必要がある。特に `grant_streak_bonus` は
  `profiles` UPDATE より前にロックを取る形へ変更しないと、共有関数側にロックを戻した瞬間に
  デッドロックが再発する。既存の金銭処理を複数書き換えるため、専用のテストと合わせて独立タスクで扱う。
