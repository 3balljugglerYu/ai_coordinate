...
- ユーザープロンプトの文字数制限を coordinate と独立に管理 (= 既存の coordinate 制約値 1000 字を踏襲)
- coordinate 側の UI 仕様変更 (= 抽出による細かな見た目調整はあり得るが、機能は不変)
- preset 単位でユーザープロンプト入力の表示制御 (= category 単位で十分、preset overide は将来要件)
...
- shadcn/ui の `<Textarea>`、`maxLength = GENERATION_PROMPT_MAX_LENGTH (1000)`
...
- user_upload は path 先頭が `temp/` になるので、第 3 の分岐 (`generated-images` bucket からの download) を追加する必要がある
- ヘルパーは既存 `downloadInputImageViaStorageFallback` を流用可能
...
現在 `skipBasePrefix` option をサポート。ユーザープロンプトは新パラメータ `userPromptInput?: string` を追加し、raw / 通常モードのどちらでも `Styling Direction` の後に `User Input:\n...` セクションとして追記する形が自然。
...
| REQ-4 | When ユーザーが入力したプロンプトと preset の stylingPrompt の両方が存在する時, the system shall **preset.stylingPrompt を先頭 + ユーザープロンプトを後続セクション (`User Input:`)** で結合して送る。<br>**EN**: When both are present, concatenate as `preset.stylingPrompt` + `\n\nUser Input:\n<userPrompt>`. |
| REQ-5 | When ユーザープロンプト入力欄が表示される時, the system shall coordinate 画面と同じ仕様 (max 1000 字 / 文字数表示 / clear ボタン) を共通コンポーネントで使う。<br>**EN**: The prompt input field shall share the coordinate UI spec via a shared component. |
...
| REQ-7 | When worker が `generation_type='one_tap_style' AND style_reference_image_url IS NOT NULL` のジョブを処理する時, the system shall path 先頭が `temp/` なら `generated-images` bucket、それ以外なら `style_presets` bucket から image_1 を取得する。<br>**EN**: For style preset jobs with reference, the worker shall route to `generated-images` bucket if path starts with `temp/`, else `style_presets`. |
...
- **Decision**: 常に `preset.stylingPrompt` を先頭 + 改行 2 つ + `User Input:\n<userPrompt>` セクションを追記して送る。category ごとに切り替えない。
- **Reason**:
  - 「preset がスタイルの方向を決め、ユーザーが細部を追加」が直感的な UX
  - 上書きにすると admin が用意した preset の意味が消える
  - category ごとの切替は admin 運用コストが上がるわりにメリットが薄い
- **Consequence**: ユーザー入力が空文字なら `User Input:` セクションを完全に省略 (= preset.stylingPrompt のみ送る)。raw モード (`skip_base_prefix=true`) でも結合形式は同じ。
...
### ADR-005: worker の bucket 判別は `style_reference_image_url` の path 先頭で行う (= 新カラム追加なし)

- **Context**: worker が image_1 の bucket を判別する仕組み。「新カラム `style_reference_image_bucket` を追加」「path 先頭で判別」の 2 案。
- **Decision**: `style_reference_image_url` (= storage path 文字列) の先頭が `temp/` なら `generated-images`、そうでなければ `style_presets` という暗黙の判別ルールを採用。inspire は generation_type で先に分岐済み。
- **Reason**:
  - 新カラム追加は migration 影響大 + analytics に不要なメタ情報
  - admin uploader は path 先頭が `{presetId}/` で固定、user uploader は `temp/{user_id}/` で固定 = 明確に判別可能
  - migration でのバックフィル不要
- **Consequence**: 将来 path 規約を変えたい時はこの判別ロジックも追従が必要。コメントで規約を明示しておく。
...
### Phase 1: DB migration (2 列追加)

- [ ] `supabase/migrations/<ts>_add_dual_reference_source_and_user_prompt.sql`:
  ```sql
  -- style_presets に dual の参考画像出所を追加
  ALTER TABLE public.style_presets
    ADD COLUMN dual_reference_source TEXT NOT NULL DEFAULT 'admin'
  ...
  ALTER TABLE public.preset_categories
    ADD COLUMN show_user_prompt_input BOOLEAN NOT NULL DEFAULT false;
  ...
  ```
- [ ] RPC `create_style_preset` / `update_style_preset` に `dual_reference_source` 引数追加 (前回 PR と同じパターンで `dual_reference_source` を DEFAULT 'admin' で受ける)
...
- [ ] `app/(app)/style/generate/handler.ts` (guest sync):
  ...
  - `category.show_user_prompt_input=true` のときは `userPrompt` form フィールドを読み、validate → `buildStyleGenerationPrompt({ ...userPromptInput })` に渡す
- [ ] `app/(app)/style/generate-async/handler.ts` (auth async):
  - 同上 + `uploadImage2` は `jobRepository.uploadSourceImage(temp_path, buffer, mimeType)` で temp upload → 返却 path を `style_reference_image_url` に保存
...
- [ ] `supabase/functions/image-gen-worker/index.ts`:
  - image_1 取得分岐 (行 1668-1704) に新条件追加:
    ```ts
      ...
      job.style_reference_image_url.startsWith("temp/")
    ) {
      // generated-images bucket の temp/ から取得
      const refImageData = await downloadInputImageViaStorageFallback(supabase, job.style_reference_image_url);
...
- [ ] worker 側は実装変更箇所 (`temp/` 分岐) を手動で確認 (Deno test は worker テストに沿う)
...
| ファイル | 操作 | 変更内容 |
|---|---|---|
| `supabase/migrations/<ts>_add_dual_reference_source_and_user_prompt.sql` | 新規 | 2 列追加 + CHECK 緩和 |
...
| `supabase/functions/image-gen-worker/index.ts` | 修正 | image_1 取得に `temp/` prefix 分岐追加 |
...
**変更概算**: 本体 ~450 行追加 / ~200 行修正、テスト ~400 行追加。
...
- [ ] **データ整合性**: `style_presets_dual_admin_requires_reference` CHECK 制約で「dual + admin なのに reference なし」を DB レベルで拒否
...
| 正常系 (handler async) | uploadSourceImage 呼び出し + style_reference_image_url に temp path 保存、user prompt が prompt_text に含まれる |
| 正常系 (worker) | temp/ prefix の path で generated-images bucket から download |
| 異常系 | user_upload で file 未提出 → 400 + 試行非消費、user prompt 1000 字超 → 400 |
...
- **DB**: migration は additive。問題時は新カラム 2 つ DROP + CHECK 制約を旧 (`style_presets_dual_requires_reference`) に戻す
- **既存データ影響**: `dual_reference_source` は backfill で 'admin' になるだけ、`show_user_prompt_input` は default false で挙動変化ゼロ
- **Edge Function**: 新分岐 (`temp/` prefix) は `style_reference_image_url` が `temp/` で始まる新規ジョブのみで効くため、旧バージョンに戻しても既存 inspire / admin dual は影響なし
...
- [x] **ER 図と migration の整合性**: `style_presets.dual_reference_source` (text/check) + `preset_categories.show_user_prompt_input` (bool) が migration と一致
- [x] **CHECK 制約と handler 検証の整合性**: 「dual + admin → reference 必須」は DB CHECK + API validation で二重防御
...
- [x] **API パラメータのソース安全性**: `uploadImage2` (File) と `userPrompt` (string) は formData から受け取り、サーバ側で MIME / 長さ検証。category.show_user_prompt_input=false なら userPrompt をサーバ側で無視
- [x] **ビジネスルールの DB 層強制**:
