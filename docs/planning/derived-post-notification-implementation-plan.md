...
- 作成日: 2026-08-04（同日改訂: 集約単位を案1→案3へ変更。ADR-001 参照）
- ステータス: レビュー待ち
...
- [ ] マイグレーション `add_derived_post_notification.sql` を作成（1ファイル、以下を含む）
  ...
  - [ ] 適用後検証 DO ブロック: (a) CHECK に新値が入ったか、(b) インデックス・トリガー2本・関数2本の存在、(c) 実在ユーザー2名を選び、原作（free・投稿済）＋派生行を実テーブルに INSERT → 1作品目の投稿で通知1件 → **同じ組で2作品目を投稿して通知が2件になる** → 1作品目を取消してその通知だけ消え2作品目の通知が残る → 自己派生でスキップされる、を確認して**検証行と通知を必ず全削除**。ユーザーが2名未満なら NOTICE でスキップ（`20260731110000` の「実在データで本体を最後まで通す」方式）
...
**変更不要（entity=派生投稿にした効果）**: `useNotifications.ts` の遷移分岐（既存の `entity_type==='post'` 汎用分岐で `/posts/{派生投稿id}` へ飛ぶ）、`server-api.ts` の `getResolvedImageId`（既存ルールで派生作品のサムネ・caption が付く）。
...
- [ ] `tests/unit/features/notifications/use-notifications.test.tsx` に追加: 新 type が汎用 post 分岐で `/posts/{entity_id}` へ遷移することを1ケース固定（将来 type 別分岐が追加されても派生通知が壊れないことの保険）
...
| `tests/unit/features/notifications/use-notifications.test.tsx` | 修正 | 遷移1ケース追加 |
...
変更しないことが設計上重要なファイル: `create_notification`（再定義せず呼ぶだけ）、`features/notifications/hooks/useNotifications.ts`・`features/notifications/lib/server-api.ts`（entity=派生投稿のため既存汎用コードのままで正しく動く）、`app/api/posts/post/route.ts`・`app/api/posts/[id]/route.ts`（API フックを置かない）、`app/api/notifications/*`・`notification-tab.ts`・`route-copy.ts`（type 非依存）。
...
DB トリガーの自動テスト基盤は無いため（1-4）、DB 部分の検証は「マイグレーション内 DO ブロック（実在データでの投稿→2件目→取消シナリオ）＋ Supabase Preview ＋ 実機」の3層で行う。
