...
stateDiagram-v2
    [*] --> Registered: 生成開始
    Registered --> Registered: 同一 File で連続生成
...
| ID | 要件 |
|---|---|
| EARS-9 | When a source image stock is linked to an `image_jobs` row before the worker creates `generated_images`, the worker shall use the latest `image_jobs.source_image_stock_id` when inserting `generated_images`.<br>**ja**: worker が `generated_images` を作成する前に `image_jobs` へ元画像ストックが紐付けられた場合、worker は INSERT 直前の最新 `image_jobs.source_image_stock_id` を使って `generated_images` を作成すること。 |
...
- **Context**: 連続生成時にダイアログを1回のみ表示するため、表示済み判定をどこで行うか。グローバルな `coordinate-source-stock-save-prompt-state` だけで判定すると、別画像の次回生成まで抑止してしまうか、逆に同一 pending batch で二重表示する可能性がある。
...
- **Consequence**: `PendingSourceImageBatch` 型も `promptShown` を含む DTO に拡張する。連続生成 → 保存/スキップ → 別画像で再生成、というフローでも正しく再表示される。

### ADR-003: 表示中保存に備えて worker 側で最新 `source_image_stock_id` を再取得する
...
- **Decision**: worker の `generated_images` INSERT 直前に `image_jobs.source_image_stock_id` を再取得し、最新値を `generated_images.source_image_stock_id` に使う。
- **Reason**: `image_jobs` を source of truth にすれば、保存タイミングが worker の処理開始前でも処理中でも結果画像に紐付けが伝播する。DB スキーマや新規 RPC は不要。
- **Consequence**: Edge Function worker に小さな SELECT が1回増える。保存導線のタイミング変更によるデータ不整合を避けられる。
...
- [ ] **ページ離脱**: 生成直後に `/stock` タブへ遷移 → 3 秒経過してもダイアログが表示されない（戻ってきても表示されない）
...
| [supabase/functions/image-gen-worker/index.ts](supabase/functions/image-gen-worker/index.ts) | 修正 | `generated_images` INSERT 直前に最新 `source_image_stock_id` を再取得 |
...
- [ ] **データ整合性**: `linkSourceImageStockToJobs` が複数 jobIds を API 上限ごとに分割更新し、worker が INSERT 直前の最新 `source_image_stock_id` を使う
- [ ] **i18n**: 既存翻訳キーを流用（追加・変更なし）
- [ ] **メモリリーク**: タイマーが unmount 時に確実にクリアされ、表示しない条件・ページ離脱・ダイアログ終了時に pending batch が消費される
- [ ] **タイミング競合**: 3秒経過直前にユーザーがアクション → アンマウントが先に走れば cleanup でクリアして batch 消費、走らなければ通常表示。worker は latest `source_image_stock_id` 再取得で保存タイミング差を吸収する
...
| ライフサイクル | アンマウントでタイマー破棄と batch 消費、同一 pending batch で1回のみ表示 |
| 実機確認 | モバイル/PC 両方、`/stock` タブ遷移で破棄 |
