...
- [x] **二重表示**: シートが開いている間、バーが絶対に描画されないこと（`sheetOpenCount` のカウンタ判定）→ ユニットテストで確認済み
  ...
  - ⭐⭐⭐ **3度目の訂正（PR #594 レビュー3巡目で判明）**: 上記「問い合わせ前に同期でnullにする」対処自体が新しい退行を生んでいた。**問い合わせそのものが失敗したとき**、既に正規に追跡していたジョブの`trackedJobId`まで失ってしまう（例: ジョブAを追跡中に、無関係な別のシートを開閉しただけでこの問い合わせが一時的なネットワーク不調で失敗すると、ジョブAはまだ進行中なのにバー・ポーリング・完了通知が失われる）。`trackedJobId`には触れず、新設した`isReconciliationPending`フラグだけでHostのポーリングを止める設計に直した。問い合わせ中は`trackedJobId`をそのまま保持し、成功時のみ書き換える（空配列なら解除・ジョブがあれば置換）。失敗時は`isReconciliationPending`を`false`に戻すだけで`trackedJobId`は保持する。あわせて`reconciliationVersion`（通し番号）を導入し、シートを立て続けに開閉して問い合わせが重複したとき、応答が発火順と異なる順序で届いても古い方が新しい方を上書きしないようにした。**同じ関数を3回連続で直す形になった**（`GenerationProgressHost.tsx`の完了検知ロジック → `sheetOpenCount`のガード漏れ → 今回の失敗時の状態喪失、といずれも「一部のケースだけ考慮して直し、別のケースを見落とす」という同型の失敗を繰り返した）
