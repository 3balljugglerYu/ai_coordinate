...
- **ADR-009 公開前のテスト生成を除外する（記録時点ゲート。20260805120000）**: 運営は公開前に admin_only カテゴリ・非公開プリセット・予約公開の期間外でテスト生成する。これが記録されると「provider 設定済みなら節目通知が運営テストで発火」「未設定でも累計を先に消費して実ユーザーの『初めて』通知が**欠落**する（節目1は一度しか交差しない）」の2重汚染になる。対策は生成された瞬間の公開状態（`sp.status='published' AND pc.visibility='public' AND pc.is_active` **かつカテゴリ表示期間 `[starts, ends)` 内**＝アプリ側の公開判定 20260610130000 と同一）を満たす生成だけを記録する記録時点ゲート。実名投稿通知は「投稿時点で公開中」に加え**「生成時点でも適格だった」ことを `was_public_at_generation=true` のイベント存在で要求**する（バックフィル行は既定 false のため、公開前テスト画像＝バックフィル時代の行を含めてローンチ後に作例投稿しても通知されない）。過去イベントは**全件残置**：`published_at IS NULL` は「未公開**または移行前データ**」（20260721120000 の定義）で、公開→draft 戻しや物理削除を経た正規利用と区別できないため自動削除はしない（特定プリセットのローンチ前掃除は運用確認のうえ preset_id 明示の個別 DELETE で。節目通知が発行済みならユニークインデックス行の削除も必要）。残置したテストイベントがあるプリセットでは「初めて」が欠落し節目も早く到達する制約が残る。節目の累計はフラグに関係なく全イベントを数える。
