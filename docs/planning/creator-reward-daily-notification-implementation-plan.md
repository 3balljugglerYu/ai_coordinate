...
- 表示: 「今日、あなたの作品が◯回利用され◯ペルコイン獲得！」（その日の累計が育つ）
...
### ADR-005: Realtime の UPDATE は「状態にあれば差分マージ、無ければ取得して差し込み」
...
- **Decision**: `event: "UPDATE"` を購読し、次のように分岐する。
  ...
  2. 状態に無い（未ロードのページから浮上してきた） → `getNotificationById` で取得して差し込み
  3. いずれの場合も未読数は**サーバーから再同期**（デバウンス）
- **Reason**:
  - 既読化・一括既読も UPDATE を発火させる。毎回 `getNotificationById` を呼ぶと、
    **一括既読で行数ぶんのリクエストが飛ぶ**。状態にある行はマージで済ませてこれを回避する。
