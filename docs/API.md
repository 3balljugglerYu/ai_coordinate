...
| `operational` | 運用用 API。現状の実装では `requireAdmin()` がなく、公開経路に載せない前提で扱う |
...
- `/api/admin/generate-webp` と `/api/admin/generate-webp-by-id` は `createAdminClient()` を使う運用 API ですが、現状の route 実装では `requireAdmin()` によるガードがありません。公開経路に露出させない前提で扱うべきです。
...
| POST | `/api/admin/generate-webp` | operational | 未生成の WebP 変換バッチを実行する |
| GET | `/api/admin/generate-webp` | operational | WebP 未生成件数を取得する |
| POST | `/api/admin/generate-webp-by-id` | operational | 特定画像に対して WebP 生成を実行する |
...
| Method | Path | Access | Summary |
| --- | --- | --- | --- |
| POST | `/api/generate-webp` | public | 画像を WebP に変換する |
