...
| `generated_images` `AFTER INSERT` | `record_style_preset_usage()` | Records One-Tap generations into append-only `style_preset_usage_events` (source of truth for milestones). **Only publicly visible generations count** (preset published × category public/active); pre-release operator tests are not recorded |
