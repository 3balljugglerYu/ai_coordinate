...
| `grant_style_preset_usage_reward` | usage trigger / reprocessor | usage event id | `void` | Rewards the One-Tap Style creator; the provider is resolved through `profiles`. Self-use, missing provider, and amount 0 settle as `skipped`. **service_role only** |
