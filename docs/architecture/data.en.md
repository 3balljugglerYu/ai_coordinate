...
| `get_grantable_free_percoin_amount` | inside each grant RPC | user id, requested amount | grantable amount | The 50k free-balance cap. **Takes a recipient-scoped advisory lock internally**, serialising every path that goes through it (both rewards, collection completion, daily post, streak, subscription). Signup/tour/referral/admin/refund grants never call it (pre-existing uncapped behaviour) |
