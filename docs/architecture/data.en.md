...
| `preset_categories` | Category metadata for One-Tap Style presets. Admins manage badge labels, raw-mode prompt behavior, default input mode, output aspect ratio (`source` or `square`), optional user-facing guidance text, `/style` control visibility, and category visibility (`public` or `admin_only`) |
