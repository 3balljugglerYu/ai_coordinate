...
| `collection_completions` | Collection-series completion and mount-generation state. Users can only select their own rows. Reservation is handled by `reserve_collection_completion()`, while completion/failure finalization is restricted to service-role route RPCs. Mount templates live in private `collection-mount-templates`; generated mounts live under `generated-images/collection-mounts/{userId}/{categoryKey}/mount-{timestamp}.png` |
