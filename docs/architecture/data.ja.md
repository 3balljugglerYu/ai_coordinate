...
| `collection_completions` | コレクションシリーズの達成・台紙生成状態。本人 SELECT のみ。公開コレクションの予約は session RPC `reserve_collection_completion()`、admin_only のアプリ経路は service role route 専用 RPC `reserve_collection_completion_for_user()`、完了/失敗確定も service role route 専用 RPC が行う。台紙テンプレは private `collection-mount-templates`、生成済み台紙は `generated-images/collection-mounts/{userId}/{categoryKey}/mount-{timestamp}.png` に保存する |
...
| `preset_categories` | One-Tap Style プリセットのカテゴリメタデータ。管理者はバッジ表示、raw モード、デフォルト入力画像モード、出力比率 (`source` / `square`)、ユーザー向け説明文、`/style` 画面の項目表示可否、公開範囲 (`public` / `admin_only`) を管理できる。book 完走ビューの見た目もここで持つ: `book_cover_overlay`(既定 `true`。`false` で表紙のタイトル等のオーバーレイを描かない＝表紙画像に文字を焼き込んだカテゴリ向け)、`book_back_cover_mode`(`default`\|`last_page`、既定 `default`。`last_page` は末尾の生成画像を裏表紙に回し固定の革表紙を出さない)。紙の比率に専用カラムは無く `output_aspect_ratio_mode` の明示比率を流用する |
