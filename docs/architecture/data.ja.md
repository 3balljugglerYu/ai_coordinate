...
| `collection_completions` | コレクションシリーズの達成・台紙生成状態。本人 SELECT のみで、予約は `reserve_collection_completion()`、完了/失敗確定は service role route 専用 RPC が行う。台紙テンプレは private `collection-mount-templates`、生成済み台紙は `generated-images/collection-mounts/{userId}/{categoryKey}/mount.png` に保存する |
