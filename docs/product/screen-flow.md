...
### 2. 生成導線(コーディネート / One-Tap Style / じゆうモード)

- `/coordinate`: うちの子コーディネート生成。未認証は 1 日 1 回のお試し生成が可能
- `/style`: One-Tap Style 生成(プリセット選択式)。`/style?style=<presetId>` で特定プリセットを初期選択
- `/free`: じゆうモード。画像アップロード + 自由記述プロンプトのみで生成(設定UIなし)。**ログイン必須**(ゲスト生成なし)・コーディネートと同額のペルコイン消費・プロンプト上限30,000文字。運営管理の錨テンプレート(キャラ保持指示)をサーバー側で前置する
- 生成結果プレビュー -> 拡大表示 -> 投稿モーダル

ポイント:

- 生成タブは「コーディネート」「One-Tap Style」「じゆう」の 3 モードをモード切替で行き来し、`NavigationBar` 上は同一タブ。
- 認証ユーザーはモデル選択・ペルコイン消費で生成。`/coordinate` のみゲストお試し対象(`/style`・`/free` は認証必須)。
- プリセットはカテゴリ単位で管理(`visibility` で公開/管理者限定、解放ゲートあり)。
- じゆうモードのページ自体は公開(SEO対象)だが、生成操作はログイン必須。錨テンプレートは admin の生成プロンプト管理(`free.base_prefix` / `free.user_direction_label`)から調整できる。
